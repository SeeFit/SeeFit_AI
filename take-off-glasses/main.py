...
def video_to_frames(input_video_path, temp_image_folder, target_fps):
    ...
    original_fps = int(cap.get(cv2.CAP_PROP_FPS)) # 30
...
if __name__ == '__main__':
    ...
    frames = video_to_frames(input_video_path, temp_image_folder, 30)

    parser = argparse.ArgumentParser()
    parser.add_argument("--input_dir", type=str, default=temp_image_folder, help="input dir")
    parser.add_argument("--save_dir", type=str, default="D:\\SeeFit_AI\\take-off-glasses\\results", help="result dir")
    ...
    # 모든 .png와 .jpg 이미지 파일들의 경로를 얻습니다.
    frames = [os.path.join(directory_path, file) for file in os.listdir(directory_path) if file.endswith(('.png', '.jpg'))]
    
    # frames 리스트를 정렬하여 순서대로 비디오를 만듭니다.
    frames.sort()

    output_video_path = 'D:\\SeeFit_AI\\take-off-glasses\\output_video.mp4'
    target_fps = 15

    frames_to_video(frames, output_video_path, target_fps)
